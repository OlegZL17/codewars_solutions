...
We've got a box glued to his dash that lights up yellow or
...
def is_interesting(number, awesome_phrases):
    ...
    check = (all_zeros, same_number, incementing, decrementing, palindrome, awesome)

    if 99 < number < 1000000000:
        if any(map(lambda x: x(number), check)):
            return 2

    for i in range(number - 2, number + 3):
        if 99 < i < 1000000000:
            if any(map(lambda x: x(i), check)):
                return 1
